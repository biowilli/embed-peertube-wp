...
async function fetchCreatorRoleInfo(id) {
  ...
  const response = await fetch(
    `${peertubeUrl}/plugins/metadata/1.8.0/router/creator/${id}`
  );
...
async function fetchOrganizationRoleInfo(id) {
  var peertubeUrl = window.peertubePlaylistData.peertubeUrl;
  const response = await fetch(
    `${peertubeUrl}/plugins/metadata/1.8.0/router/organization/${id}`
  );
  const data = await response.json();
  return data;
}
...
function updateMetadata(targetUuid) {
  ...
  var peertubeUrl = window.peertubePlaylistData.peertubeUrl;
  fetch(`${peertubeUrl}/plugins/metadata/1.8.0/router/metadata/${targetUuid}`)
    .then((response) => {
      if (!response.ok) {
